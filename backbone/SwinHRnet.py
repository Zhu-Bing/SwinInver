...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_momentum=0.1):
        ...
        self.norm1 = nn.InstanceNorm3d(planes, momentum=norm_momentum)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.norm2 = nn.InstanceNorm3d(planes, momentum=norm_momentum)
        self.conv3 = nn.Conv3d(planes, planes * self.expansion, kernel_size=1, bias=False)
        self.norm3 = nn.InstanceNorm3d(planes * self.expansion, momentum=norm_momentum)
...
class StageModule(nn.Module):
    def __init__(self, stage, output_branches, c, norm_momentum):
        ...
        window_size = ensure_tuple_rep(4, 3)
        self.window_size = window_size
        for i in range(self.stage):
            w = c * (2 ** i)
            num_heads = 4 * (2**i)
            # branch = SwinTransformer(
            #     in_chans=w,
            #     embed_dim=w,
            #     window_size=self.window_size,
            #     patch_size=1,
            ...
            for j in range(self.stage):  # for each branch
                if i == j:
                    ...
                    self.fuse_layers[-1].append(nn.Sequential(
                        ...
                        nn.InstanceNorm3d(c * (2 ** i), eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
                    ...
                    for k in range(i - j - 1):
                        ops.append(nn.Sequential(
                            nn.Conv3d(c * (2 ** j), c * (2 ** j), kernel_size=3, stride=2, padding=1,
                                      bias=False),
                            nn.InstanceNorm3d(c * (2 ** j), eps=1e-05, momentum=0.1, affine=True,
...
class SwinHRNet(nn.Module):
    def __init__(self, base=32, norm_momentum=0.1):
        super(SwinHRNet, self).__init__()
        c = base
        ...
        self.conv_block1 = nn.Sequential(
            ...
            nn.InstanceNorm3d(64, eps=1e-05, momentum=norm_momentum, affine=True,
                           track_running_stats=True),
            nn.ReLU(inplace=True),
            BasicBlock(64, 64))
        ...
        downsample = nn.Sequential(
            ...
            nn.InstanceNorm3d(256, eps=1e-05, momentum=norm_momentum, affine=True, track_running_stats=True),
        )
        ...
        self.transition1 = nn.ModuleList([
            nn.Sequential(
                ...
                nn.InstanceNorm3d(c, eps=1e-05, momentum=norm_momentum, affine=True, track_running_stats=True),
                nn.ReLU(inplace=True),
            ),
            nn.Sequential(nn.Sequential(  # Double Sequential to fit with official pretrained weights
                nn.Conv3d(256, c * (2 ** 1), kernel_size=3, stride=2, padding=1, bias=False),
                nn.InstanceNorm3d(c * (2 ** 1), eps=1e-05, momentum=norm_momentum, affine=True,
                               track_running_stats=True),
                nn.ReLU(inplace=True),
            )),
        ])
        ...
        self.transition2 = nn.ModuleList([
            ...
            nn.Sequential(nn.Sequential(  # Double Sequential to fit with official pretrained weights
                ...
                nn.InstanceNorm3d(c * (2 ** 2), eps=1e-05, momentum=norm_momentum, affine=True,
                               track_running_stats=True),
                nn.ReLU(inplace=True),
        ...
        #        nn.InstanceNorm3d(c * (2 ** 3), eps=1e-05, momentum=norm_momentum, affine=True,
        #                          track_running_stats=True),
        #        nn.ReLU(inplace=True),
        #     )),  # ToDo Why the new branch derives from the "upper" branch only?
        # ])
        ...
        self.fuse_up_layer1 = nn.Sequential(nn.Conv3d(c * 7, c * 4, kernel_size=3, stride=1, padding=1),
                                            nn.InstanceNorm3d(c * 4, eps=1e-05, momentum=norm_momentum, affine=True,
                                                           track_running_stats=True),
                                            nn.ReLU(inplace=True),
                                            nn.ConvTranspose3d(c * 4, c * 2, 2, 2),
                                            nn.InstanceNorm3d(c * 2, eps=1e-05, momentum=norm_momentum, affine=True,
                                                           track_running_stats=True),
                                            nn.ReLU(inplace=True),
                                            BasicBlock(c * 2, c * 2)
                                            )
        self.fuse_up_layer2 = nn.Sequential(nn.Conv3d(c * 2 + 64, c * 2, kernel_size=3, stride=1, padding=1),
                                            ...
                                            nn.InstanceNorm3d(c, eps=1e-05, momentum=norm_momentum, affine=True,
                                                           track_running_stats=True),
                                            nn.ReLU(inplace=True))
