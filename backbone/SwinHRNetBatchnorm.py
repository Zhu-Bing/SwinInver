...
#         super().__init__()
#         self.conv1 = nn.Conv3d(c * (2 ** j), c * (2 ** j), kernel_size=1, stride=1, padding=0,)
#         self.norm1 = nn.BatchNorm3d(c * (2 ** j), eps=1e-05, momentum=0.1, affine=True,track_running_stats=True)
#         self.conv2 = nn.Conv3d(c * (2 ** j), c * (2 ** j), kernel_size=3, stride=1, padding=2,dilation=2,bias=False)
#         self.norm2 = nn.BatchNorm3d(c * (2 ** j), eps=1e-05, momentum=0.1, affine=True,track_running_stats=True)
#         self.conv3 = nn.Conv3d(c * (2 ** j), c * (2 ** j), kernel_size=1, stride=2, padding=0,bias=False)
#         self.norm3 = nn.BatchNorm3d(c * (2 ** j), eps=1e-05, momentum=0.1, affine=True,track_running_stats=True)
#         self.relu = nn.ReLU(inplace=True)
#         self.softpool = SoftPool3d()
#
#
#
#     def forward(self,x):
...
#         out = self.relu(out)
#
#         x = out + self.softpool(x)
#         return x

class SwinBasicblock(nn.Module):
    def __init__(self,dim, num_heads , window_size,inplanes, planes, stride=1, downsample=None, norm_momentum=0.1):
        super().__init__()
    ...
    def __init__(self, stage, output_branches, c, norm_momentum):
        ...
        for i in range(self.stage):
            ...
            branch = nn.Sequential(
                # SwinBasicblock(dim=w, num_heads=num_heads, window_size=self.window_size,inplanes=w,planes=w),
                BasicLayer(
                    dim=w,
                    depth=2,
                    num_heads=num_heads,
...
class SwinHRNetB(nn.Module):
    def __init__(self, base=32, norm_momentum=0.1):
        super(SwinHRNetB, self).__init__()
        c = base
