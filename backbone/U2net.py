...
class ConvBNReLU(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, kernel_size: int = 3, dilation: int = 1):
        ...
        self.conv = nn.Conv3d(in_ch, out_ch, kernel_size, padding=padding, dilation=dilation, bias=False)
...
class DownConvBNReLU(ConvBNReLU):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.down_flag:
            x = F.max_pool3d(x, kernel_size=2, stride=2, ceil_mode=True)
...
class U2Net(nn.Module):
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        for c in cfg["encode"]:
            ...
            if c[5] is True:
                side_list.append(nn.Conv3d(c[3], out_ch, kernel_size=3, padding=1))
        self.encode_modules = nn.ModuleList(encode_list)
        ...
        self.out_conv = nn.Conv3d(self.encode_num * out_ch, out_ch, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        for m in self.side_modules:
            ...
            x = F.interpolate(m(x), size=[t, h, w], mode='trilinear', align_corners=False)
